	...
	const char* file_)
{
	...
	if (!infile.is_open())
	{
		std::cerr <<
			...
			file_ << "\", The file does not exist." << std::endl;
		return false;
	}
	...
	while (getline(infile, line))
	{
			...
			loaded_coordinates.back().size() != split_elements.size())
		{
			std::cerr <<
				"A problem occured when loading the points from file \"" <<
				file_ << "\". The number of coordinates vary in the rows." << std::endl;
			return false;
		}
	...
	std::cout <<
		"Number of loaded correspondences = " << point_number << std::endl;
...
void getLabeling(
	...
	const double &neighborhoodRadius_,
	const double& inlierOutlierThreshold_,
	const double& spatialWeight_,
	const double& labelCost_,
	std::vector<size_t> &labels_,
	size_t &max_labels_,
	bool doLogging_ = false)
{
	const size_t kPointNumber = points_.rows;
	const size_t kModelNumber = models_.size();
	const double kSquaredThreshold = inlierOutlierThreshold_ * inlierOutlierThreshold_;
	_ModelEstimator estimator;

	std::chrono::time_point<std::chrono::system_clock> start, end; // Variables for time measurement
	start = std::chrono::system_clock::now(); // The starting time of the neighborhood calculation
	gcransac::neighborhood::BruteForceNeighborhoodGraph neighborhood(&points_, neighborhoodRadius_);
	...
	if (doLogging_)
		printf("Brute-force neighborhood calculation time: %f secs.\n", elapsed_seconds.count());

	std::vector<size_t> inlierNumbers(kModelNumber, 0);
	std::vector<double> minDistances(
		points_.rows,
		std::numeric_limits<double>::max());
	
	pearl::PEARL<
		gcransac::neighborhood::BruteForceNeighborhoodGraph, 
		_ModelEstimator> pearl(
			inlierOutlierThreshold_,
			spatialWeight_,
			_ModelEstimator::sampleSize(),
			labelCost_);
	double energy = 0;
	...
	if (doLogging_)
		printf("Labeling time: %f secs.\n", elapsed_seconds.count());

	labels_ = pearl.getLabeling(max_labels_);
}
	...
	int K_annot)
{
	...
	for (int i = 0; i < models.size(); ++i)
	{
		for (int j = 0; j < annotation.size(); ++j)
			if (models[i].consensusVector[j] > FLT_EPSILON)
				preferences[i][j] = 1;
	}
	...
	int K_annot)
{
	...
	std::vector<int> gtPair(K_annot + 1, 0);
...
inline void projectDataToRDimensionalSpace(
	const cv::Mat &points_,
	cv::Mat& projectedPoints_, 
	const int r_)
{
	cv::Mat S, U, Vt;
	cv::SVD::compute(points_, S, U, Vt, cv::SVD::FULL_UV);

	projectedPoints_.create(points_.rows, r_, CV_64F);

	for (int i = 0; i < r_; ++i)
		U.col(i).copyTo(projectedPoints_.col(i));
	projectedPoints_ = projectedPoints_.t();
}

inline void readAnnotatedPointSequence(
	...
	std::vector<int>& labels_)
{
	...
	infile >> frames_ >> point_number;

	points_.create(point_number, 2 * frames_, CV_64F);
	labels_.resize(point_number);
	...
	while (infile >> x >> y)
	{
		points_.at<double>(pt_counter, 2 * fr_counter) = x;
		points_.at<double>(pt_counter, 2 * fr_counter + 1) = y;

		if (++fr_counter >= frames_)
		{
			int a;
			infile >> a;
			labels_[pt_counter] = a;
