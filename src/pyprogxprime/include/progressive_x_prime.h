...
namespace progx
{
	...
	template<
		...
		void ProgressiveXPrime<_Clustering, _ModelModelDistance, _RobustLoss, _ModelEstimator, _Sampler, _ScoringFunction>::run(
			const cv::Mat& points_,
			_Sampler& sampler_,
			std::vector<gcransac::Model>& models_,
			std::vector<ModelData>& modelData_) const
	{
		printf("Running Progressive-X*\n");
		...
		while (running)
		{
			//printf("Unseen inlier number prediction = %d.\n", maxInlierNumber);
	...
	template<
		...
		class _Sampler, // The sampler used in the main RANSAC loop of GC-RANSAC
		class _ScoringFunction>
		bool ProgressiveXPrime<_Clustering, _ModelModelDistance, _RobustLoss, _ModelEstimator, _Sampler, _ScoringFunction>::generateHypotheses(
			const cv::Mat& points_,
			const size_t& nextHypothesesNumber_,
			std::vector<size_t>& pointPool_,
			const std::vector<double>& compoundModel_,
	...
	template<
		...
		class _Sampler, // The sampler used in the main RANSAC loop of GC-RANSAC
		class _ScoringFunction>
	void ProgressiveXPrime<_Clustering, _ModelModelDistance, _RobustLoss, _ModelEstimator, _Sampler, _ScoringFunction>::refitAndReplace(
		std::vector<ModelData>& hypothesisData_,
		std::vector<gcransac::Model>& hypotheses_,
		const _ModelEstimator& estimator_,
		const cv::Mat& points_,
	...
	template<
		...
		class _Sampler, // The sampler used in the main RANSAC loop of GC-RANSAC
		class _ScoringFunction>
	void ProgressiveXPrime<_Clustering, _ModelModelDistance, _RobustLoss, _ModelEstimator, _Sampler, _ScoringFunction>::iterativelyReweightedLSQ(
		const cv::Mat& points_,
		const _ModelEstimator& estimator_,
		gcransac::Model& model_,
		std::vector<size_t>& inliers_) const
	...
	template<
		...
		class _Sampler, // The sampler used in the main RANSAC loop of GC-RANSAC
		class _ScoringFunction>
	void ProgressiveXPrime<_Clustering, _ModelModelDistance, _RobustLoss, _ModelEstimator, _Sampler, _ScoringFunction>::extractInliersFromClusters(
		const cv::Mat& points_,
		const std::vector<ModelData>& hypothesisData_,
		const std::vector< std::vector<size_t> >& clusterIndices_,
		std::vector<std::pair<std::vector<size_t>, size_t>> &clusterInliers_) const
	...
	template<
		...
		class _Sampler, // The sampler used in the main RANSAC loop of GC-RANSAC
		class _ScoringFunction>
	void ProgressiveXPrime<_Clustering, _ModelModelDistance, _RobustLoss, _ModelEstimator, _Sampler, _ScoringFunction>::displayClustering(
		const cv::Mat& points_,
		const std::vector<ModelData>& hypothesisData_,
		const std::vector< std::vector<size_t> >& clusterIndices_) const
	{
